...
export default function Driver() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && data.numeric_id) {
        setDriverId(data.numeric_id.toString());
      }
      setMode('dashboard');
      setIsAvailable(true);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        const driverStatus = await getDriverStatus(parseInt(driverId.replace('DRIVER-', '')));
        if (driverStatus) {
          // status updated
        }

        // Check for pending ride offer
        const pendingRide = await getDriverPendingRide(parseInt(driverId.replace('DRIVER-', '')));
        if (pendingRide && pendingRide.has_ride && !currentRide) {
          setCurrentRide({
            ...
            fare: pendingRide.fare ? `₹${pendingRide.fare}` : "₹100",
  ...
  // Heartbeat — keeps the driver "online" in DB so the matcher picks them up
  useEffect(() => {
    if (!isAvailable || !driverId) return;
    sendDriverHeartbeat(driverId); // immediate on go-online
    const heartbeat = setInterval(() => sendDriverHeartbeat(driverId), 15000);
  ...
  const acceptRide = async () => {
    ...
    try {
      await acceptRideRequest(parseInt(driverId.replace('DRIVER-', '')), currentRide.match_id);
  ...
  const declineRide = async () => {
    if (!currentRide || !currentRide.match_id) return;
    try {
      await declineRideRequest(parseInt(driverId.replace('DRIVER-', '')), currentRide.match_id);
      setCurrentRide(null);
    } catch (error) {
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 relative overflow-hidden">
        {activeTab === 'school' ? (
          <DriverSchoolTab driverId={parseInt(driverId.replace('DRIVER-', ''))} />
        ) : (
          <>
            <MapView
              ...
              drivers={[{
                driver_id: parseInt(driverId.replace('DRIVER-', '')) || 0,
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-white p-6 rounded-t-3xl shadow-2xl">
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      await completeRide(parseInt(driverId.replace('DRIVER-', '')), currentRide.id);
                      setCurrentRide(null);
                      setOtpInput('');
