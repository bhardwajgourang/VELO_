...
# Auth Configuration — must match token_manager (auth/tokens.py)
...
def signup(user: UserCreate, db: Session = Depends(get_db)):
    ...
    return {"access_token": token_pair.access_token, "refresh_token": token_pair.refresh_token, "token_type": "bearer", "expires_in": token_pair.expires_in}
...
def refresh_token_endpoint(req: TokenRefreshRequest, db: Session = Depends(get_db)):
    ...
    try:
        is_valid, payload = token_manager.validate_refresh_token(req.refresh_token)
...
def set_driver_availability(driver_id: str, is_available: bool, db: Session = Depends(get_db)):
    try:
        ...
        driver.updated_at = datetime.utcnow()  # Refresh heartbeat on availability change
        db.commit()
...
@app.post("/driver/heartbeat")
def driver_heartbeat(driver_id: str, db: Session = Depends(get_db)):
    """Called by the driver app every 15s to signal it is still online."""
    try:
        numeric_id = int(driver_id.replace("DRIVER-", ""))
        driver = db.query(DriverInfo).filter(DriverInfo.driver_id == numeric_id).first()
        if not driver:
        ...
        if driver.available:
            driver.updated_at = datetime.utcnow()
            db.commit()
        return {"ok": True, "driver_id": numeric_id, "available": driver.available}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
